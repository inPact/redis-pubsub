...
class Subscriber extends EventEmitter {
    constructor(redis) {
        ...
        let onMessage = function (pattern, channel, message) {
            debug(`SUBSCRIBER: received pmessage on channel ${channel}`);
        ...
        this.redis.on('psubscribe', (pattern, count) => debug(`SUBSCRIBER: psubscribe: ${count} subscribers on pattern ${pattern}`));
        this.redis.on('punsubscribe', (pattern, count) => debug(`SUBSCRIBER: punsubscribe: ${count} subscribers on pattern ${pattern}`));
    }
