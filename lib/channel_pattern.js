...
class ChannelPattern {
     ...
     * If the registered pattern was 'a.b.c.d' and the published topic was 'a.b.c.d', the event
     * part of the path defaults to the final topic part, in this case 'd'.
     * @returns {String}
     */
    get event() {
        let relevantTopic = this.subTopics[0];

        if (relevantTopic)
            return relevantTopic;

        if (this.channel)
            return _.last(this.channel.split('.'));
